...
MAP = {
    ...
    'arm_btn':     evdev.ecodes.BTN_TL,     # L1 held = dead-man arm
    # Re-arm: both thumbsticks clicked simultaneously
    'rearm_combo':  (evdev.ecodes.BTN_THUMBL, evdev.ecodes.BTN_THUMBR),
}

# Physical face-button labels for G3 V2 (Nintendo-style layout).
# evdev Xbox convention: SOUTH=A, EAST=B, NORTH=Y, WEST=X
# G3 V2 physical layout: SOUTH=B, EAST=A, NORTH=X, WEST=Y
_BTN_DISPLAY_NAMES = {
    evdev.ecodes.BTN_SOUTH: 'B',
    evdev.ecodes.BTN_EAST:  'A',
...
def get_abs_info(dev):
    return dev.capabilities(absinfo=True).get(evdev.ecodes.EV_ABS, [])


def get_btn_codes(dev):
    all_keys = dev.capabilities().get(evdev.ecodes.EV_KEY, [])
    return sorted(c for c in all_keys if c >= evdev.ecodes.BTN_MISC)
